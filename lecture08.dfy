...
method M8a()
{
    // על הלוח ציירנו את זה מלמטה למעלה עם קשתות לא מכוונות (כי מסכימים שהכיוון הוא מעלה), ללא קשתות עצמיות, וללא קשתות עקיפות
    assert {} <= {1} && {} <= {2} && {} <= {3};
    assert {1} <= {1,2} && {1} <= {1,3};
    assert {2} <= {1,2} && {2} <= {2,3};
    assert {3} <= {1,3} && {3} <= {2,3};
...
יחס מילוני
*/
function LexicographicOrder<T1(!new),T2(!new)>(R: BinaryRelation<T1,T1>, S: BinaryRelation<T2,T2>, A: iset<T1>, B: iset<T2>): BinaryRelation<(T1,T2),(T1,T2)>
...
/*
דוגמה על הקשר בין הגדרת פונקציה לבין התפיסה המקובלת: למשל
x^2 + 3x = y
זו למעשה לא פונקציה אלא משוואה; ניתן לייצג אותה (על הממשיים) באופן הבא
*/
...
*/
function RelationOnFunctionDomain<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>, A: iset<T1>):  BinaryRelation<T1,T1>
...
where B is the set of all equivalence classes A/E
...
קושר בין שני איברים של
A
אם הפונקציה לוקחת אותם לאותה תוצאה
כלומר לאותה מחלקת שקילות
והרי זה בדיוק מה שיחס השקילות
E
עושה: שני איברים של 
A
נמצאים ב
E
אם"ם שניהם באותה מחלקת שקילות
...
*/
function ReducedFunction<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>, A0: iset<T1>):  BinaryRelation<T1,T2>
...
/*
מה לגבי חיתוך ואיחוד של פונקציות? האיחוד אינו בהכרח פונקציה, החיתוך כן אבל על תחום חלקי

האיחוד של פונקציה מהווה פונקציה אם ורק אם
F|`(dom(F)*dom(G) = G|`(dom(F)*dom(G)

בפרט אם התחומים זרים, האיחוד הוא בוודאות פונקציה
...
predicate UnifiableFunctions<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>, G:  BinaryRelation<T1,T2>)
    requires ValidFunction(F)
    requires ValidFunction(G)
{
...
predicate EquivalentFunctionDefinitions'<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>, G: T1 -> T2)
    requires ValidFunction(F)
{
    forall x,y :: (x,y) in F <==> y == G(x)
}

predicate InjectiveFunction'<T1(!new),T2(!new)>(F: T1 -> T2)
{
...
predicate SurjectiveFunction''<T1(!new),T2(!new)>(F: T1 -> T2, A: iset<T1>, B: iset<T2>)
    requires A == Domain'(F) && Range'(F) <= B
{
