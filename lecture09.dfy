...
רמות שונות של אינסוף: שקילות עוצמתית של קבוצות + קבוצות סופיות + קבוצות אינסופיות ניתנות למניה + וכאלה שאינן

משפט קנטור + שיטת הליכסון של קנטור
...
*/
function TheCardinallyEquivalentSetsRelation<T(!new)>(SetOfAllSets: iset<iset<T>>): BinaryRelation<iset<T>,iset<T>>
...
כהכנה להגדרת סופיות של קבוצה נגדיר סימון

A^<n

לקבוצת כל הפונקציות מהטבעיים הקטנים מ
n
אל איברים מהקבוצה
A
...
function AllSequencesShorterThan<T(!new)>(n: nat, A: iset<T>): iset<Function<nat, T>>
{
    iset Q: Function<nat, T>, m: nat | m < n && ValidSequenceShorterThan(m, Q, A) :: Q
}

predicate ValidSequenceShorterThan<T(!new)>(n: nat, Q: Function<nat, T>, A: iset<T>)
{
...
function AllNaturalNumberSequencesShorterThan(n: nat): iset<Function<nat, nat>>
{
...
predicate IsFinite<T(!new)>(X: iset<T>)
{
    ...
    ensures IsFinite(ISetOf(Phi))
{
    ...
    == // by definition
        exists F: Function<T, Function<nat, nat>> :: ValidFunctionDomain(F, emptySet) && 
        Range(F) <= emptySetOfFunctions && BijectiveFunction(F, emptySet, emptySetOfFunctions);
...
הגדרה: קבוצה
A
נקראת בת מניה (ניתנת למניה) אם
A
סופית או
A ~ N

*/
predicate IsCountable<T(!new)>(A: iset<T>)
{
    IsFinite(A) || CardinallyEquivalentSets(A, AllNaturalNumbers)
}

/*
משפט: התנאים הבאים על קבוצה
A
שקולים זה לזה
...
/*
מעניין שניתן להוכיח בצורה מעגלית
top LHS ==> top RHS,
top RHS ==> bottom RHS
bottom RHS ==> top LHS

מעגליות הגרירות מבטיחה שקילות בין כל 3 הטענות, כנדרש
*/


/*

תורת הקבוצות מאפשרת הגדרה של חשבון מונים (בעיקר אינסופיים)
...
/*
ניתן להראות בעזרת פונקציה חח"ע מהמכפלה ל
N
למשל
2^a*3^b
בזכות יחידות הפירוק לגורמים ראשוניים
*/

/*
טענה: גם איחוד של קבוצות בנות מניה הוא בן מניה

ניתן להוכיח על ידי הרכבה של פונקציה לזוגיים ועוד פונקציה לאי-זוגיים
*/
...
/*
איחוד בן מניה של קבוצות בנות מניה גם הוא בן מניה

אם
A,B
קבוצות בנות מניה, אז גם
A X B
קב' בת מניה
*/
...
lemma {:verify true} Cantor5'<T(!new)>(F: Function<T, iset<T>>, A: iset<T>, B: iset<iset<T>>)
    ...
    ensures Range(F) < B
{
    var X := iset a0, F_of_a0 | a0 in A && (a0, F_of_a0) in F && a0 !in F_of_a0 :: a0; // Canto's Diagonalization
...
מסקנה: קבוצת המספרים הטבעיים "קטנה ממש" מקבוצת החזקה של הטבעיים

ניתן "לשכן" את הטבעיים בקבוצת החזקה שלהם אך
קבוצת הטבעיים אינה "שקולה עוצמתית" לקבוצת החזקה של הטבעיים
...
/*
מסקנת המשך: קבוצת החזקה של המספרים הטבעיים אינה ניתנת למניה
*/
lemma PowersetOfNaturalNumbersIsNotCountable()
    ensures !IsCountable(Powerset(AllNaturalNumbers))

/*
ממשפט קנטור בשילוב עם טענות נוספות (שלא נוכיח כאן) ניתן לראות שיש "יותר" מספרים ממשיים מטבעיים: קבוצות המספרים הללו אינן שקולות עוצמתית
ומכאן גם שקבוצת הממשיים אינה ניתנת למניה
*/
...
/* 
מסתבר שהממשיים שקולים עוצמתית לקבוצת החזקה של הטבעיים, שכאמור איננה ניתנת למניה
*/
