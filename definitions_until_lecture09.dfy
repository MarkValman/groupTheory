// from lecture02.dfy
function SetUnion<T(!new)>(A: iset<T>, B: iset<T>): iset<T>
{
...
predicate IsStrictSubset(A: set, B: set) // "<" ("הכלה ממש")
{
	IsSubset(A, B) &&
...
predicate IsSuperset(A: iset, B: iset)
{
...
predicate RelationOn<T(!new)>(R: BinaryRelation<T,T>, A: iset<T>)
{
	forall a,b :: (a,b) in R ==> a in A && b in A // forall p :: p in R ==> p.0 in A && p.1 in A
}

predicate Transitive<T(!new)>(R: BinaryRelation<T,T>) { forall a, b, c :: (a,b) in R && (b,c) in R ==> (a,c) in R }
predicate NonTransitive<T(!new)>(R: BinaryRelation<T,T>) { exists a, b, c :: (a,b) in R && (b,c) in R && (a,c) !in R }

// from lecture06.dfy

predicate Reflexive<T(!new)>(R: BinaryRelation<T,T>, A: iset<T>)
    requires RelationOn(R, A)
{
    forall a :: a in A ==> (a,a) in R
}

predicate NonReflexive<T(!new)>(R: BinaryRelation<T,T>, A: iset<T>)
    requires RelationOn(R, A)
{
    exists a :: a in A && (a,a) !in R
}

predicate Symmetric<T(!new)>(R: BinaryRelation<T,T>) { forall a: T, b: T :: (a,b) in R <==> (b, a) in R }

predicate NonSymmetric<T(!new)>(R: BinaryRelation<T,T>) { exists a: T, b: T :: (a,b) in R && (b,a) !in R }

predicate EquivalenceRelation<T(!new)>(R: BinaryRelation<T,T>, A: iset<T>)
    requires RelationOn(R, A)
{
    Transitive(R) && Reflexive(R, A) && Symmetric(R)
}

function EquivalenceClassOf<T>(a: T, A: iset<T>, R: BinaryRelation<T,T>): iset<T>
...
function UnaryUnion<T>(As: iset<iset<T>>): iset<T>
{
    iset a,A | A in As && a in A :: a
}

predicate PairwiseDisjoint<T>(As: iset<iset<T>>)
{
    forall A,B :: A in As && B in As && A != B ==> A !! B // "A !! B" is short for "disjoint": A*B == iset{}
}

predicate NonEmptyISets<T>(As: iset<iset<T>>)
{
    forall A :: A in As ==> A != iset{}
}

predicate IsPartition<T>(P: iset<iset<T>>, A: iset<T>)
{
    NonEmptyISets(P) && PairwiseDisjoint(P) && UnaryUnion(P) == A
}

function AllEquivalenceClasses<T>(A: iset<T>, R: BinaryRelation<T,T>): iset<iset<T>>
    requires RelationOn(R, A) && EquivalenceRelation(R, A)
{
    iset a | a in A :: EquivalenceClassOf(a, A, R)    
}

predicate AntiSymmetric<T(!new)>(R: BinaryRelation<T,T>) { forall a: T, b: T :: (a,b) in R && (b,a) in R ==> a == b }

predicate IsPartialOrder<T(!new)>(R: BinaryRelation<T,T>, A: iset<T>)
    requires RelationOn(R, A)
{
    Transitive(R) && Reflexive(R, A) && AntiSymmetric(R)
}

function Domain<T1(!new),T2(!new)>(R: BinaryRelation<T1,T2>): iset<T1> { iset pair | pair in R :: pair.0 } // תחום
function Range<T1(!new),T2(!new)>(R: BinaryRelation<T1,T2>): iset<T2> { iset pair | pair in R :: pair.1 } // טווח/תמונה
function isetOf<T(!new)>(R: BinaryRelation<T,T>): iset<T> { Range(R) + Domain(R) }

type PartialOrder<!T(!new,==)> = R: BinaryRelation<T,T> | var A := isetOf(R); RelationOn(R, A)&& IsPartialOrder(R, A)

function Powerset<T>(A: iset<T>): iset<iset<T>> // קבוצת החזקה
{
	iset B | B <= A
}

function CartesianProduct<T1,T2>(A: iset<T1>, B: iset<T2>): iset<(T1,T2)>
{
...
function SubsetRelation(A: iset): BinaryRelation<iset, iset>
{
...
predicate IsTotalOrder<T(!new)>(R: BinaryRelation<T,T>, A: iset<T>)
    requires RelationOn(R, A)
{
...
predicate IsTotalOrder'<T(!new)>(R: PartialOrder<T>, A: set<T>)
{
	forall a,b :: a in A && b in A ==> (a,b) in R || (b,a) in R
}

function InverseRelation<T1(!new),T2(!new)>(R: BinaryRelation<T1,T2>): BinaryRelation<T2,T1>
{
    iset x,y | x in Domain(R) && y in Range(R) && (x,y) in R :: (y,x)
}

function IdentityRelation<T>(A: iset<T>): BinaryRelation<T,T>
{
	iset a | a in A :: (a, a)
}
...
predicate ValidFunction<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>)
{
...
*/
predicate ValidFunction'<T1(!new),T2(!new)>(F: iset<(T1,T2)>)
{
    forall p1,p2 :: p1 in F && p2 in F && p1.0 == p2.0 ==> p1.1 == p2.1
}

predicate ValidFunctionDomain<T1(!new),T2(!new)>(F: Function<T1,T2>, A: iset<T1>)
{
    A == Domain(F)
}

predicate InjectiveFunction<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>)
{
    ValidFunction(F) &&
    forall x1,x2,y1,y2 :: (x1,y1) in F && (x2,y2) in F && x1 != x2 ==> y1 != y2
}

predicate SurjectiveFunction<T1(!new),T2(!new)>(F:  BinaryRelation<T1,T2>, A: iset<T1>, B: iset<T2>)
...
predicate SurjectiveFunction'<T1(!new),T2(!new)>(F: Function<T1,T2>, A: iset<T1>, B: iset<T2>)
...
/* חח"ע ועל */
predicate BijectiveFunction<T1(!new),T2(!new)>(F: Function<T1,T2>, A: iset<T1>, B: iset<T2>)
    requires A == Domain(F) && Range(F) <= B
{
...
/*
הגדרת שקילות עוצמתית
A~B if there exists an injective+surjective (i.e. bijective) F:A->B
*/
predicate CardinallyEquivalentSets<T1(!new),T2(!new)>(A: iset<T1>, B: iset<T2>)
{
