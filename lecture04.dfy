...
- proving bidirectional equivalence (logical equivalence, if-and-only-if, iff, אם"ם): (L ==> R && L <== R), (L ==> R && !L == !R)
    ...
    ensures A == B
{
    ...
    assert Alef: forall x :: x in A ==> x in B by {
        forall x | x in A ensures x in B {
            assert Alef1: x in A;
            if x !in B // נניח בשלילה
            {
                assert Alef2: x !in A by { reveal Pre1; }
                assert false by { // ונוביל לסתירה
                    assert x in A && x !in A by { reveal Alef1,Alef2; }
                }
                // למעשה לא השתמשנו בהנחה, הסתירה היתה מעצם בחירת איבר מקבוצה ריקה
            }
        }
    }
    assert Bet: forall x :: x in B ==> x in A by {
        forall x | x in B ensures x in A { // similar to the above, written more directly
            assert Bet1: x in B;
            assert Bet2: false by { // ונוביל לסתירה
...
// proving bidirectional equivalence (logical equivalence, if-and-only-if, iff, אם"ם): L ==> R, R ==> L
/*
