...
predicate IsStrictSubset2(A: set, B: set) // "<" ("הכלה ממש")
{
	IsSubset(A, B) &&
	SetDifference(B, A) != {} 
}
...
predicate IsStrictSubset3(A: set, B: set)
{
	IsSubset(A, B) &&
	|SetDifference(B, A)| != 0 
}

// and could also be written also this way:
predicate IsStrictSubset4(A: set, B: set)
{
	IsSubset(A, B) &&
	|A| < |B| 
}
...
// or this way:
predicate IsStrictSubset5(A: set, B: set)
{
	IsSubset(A, B) &&
...
lemma {:verify false} EquivalentDefinitionsOfStrictSubset(A: set, B: set)
	ensures IsStrictSubset(A, B) == IsStrictSubset2(A, B) == IsStrictSubset3(A, B) == IsStrictSubset4(A, B)
{}
...
lemma NotSelfStrictSubset(A: set)
    ensures !IsStrictSubset(A, A)
{}
...
lemma {:verify false} DeMorganOfForall(A: set<int>)
	ensures !(forall x :: x in A <==> x%2 == 0) <==> (exists y :: y in A && y%2 != 0)
{}